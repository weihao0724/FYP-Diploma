...
if(isset($_POST["savebtn"]))
{
	...
	if(empty($id_check) && empty($email_check) && empty($phone_check) && empty($age_check) && empty($name_check) && empty($password_check) && empty($conpw_check))
	{
		...
		?>
			<script>alert("<?php echo "$param_name is saved as an admin.";?>");</script>
		<?php
	...
	{
		?>
			<script>console.log("Something went wrong. New admin cannot be saved.")</script>
		<?php
	}
}
...
<!--div for adding a new admin-->
		<div class="main-body">
			...
			<div id="add-admin" class="">
				...
				<form name="addfrm" method="post" class="addform" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
					<p class="<?php echo (!empty($id_check)) ? 'has-error' : ''; ?>">
						<label>ID (need 6 characters):&nbsp;</label>
						<input type="text" name="staff_id" value="<?php echo $id; ?>">
					...
					<p class="<?php echo (!empty($password_check)) ? 'has-error' : ''; ?>">
						...
						<span id="valid-pass"><?php echo $password_check; ?></span><br>
